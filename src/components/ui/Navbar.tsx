...
export default function Navbar() {
  ...
  const linkStyles = (path: string) => `
    text-[10px] md:text-xs font-bold tracking-[0.15em] uppercase transition-all duration-300
    ${isActive(path) 
      ? "text-blue-600 scale-110 drop-shadow-sm" 
      : "text-slate-500 hover:text-slate-900 hover:scale-110"}
  `;
  ...
  return (
    ...
    <motion.nav 
      ...
      transition={{ type: "spring", stiffness: 100, damping: 20, delay: 0.5 }} // Delay para que no compita con el Hero
      className="w-full flex flex-col md:flex-row items-center justify-between py-6 md:py-8"
    >
      
      {/* LOGO */}
      <div className="mb-5 md:mb-0 text-center md:text-left w-full md:w-auto">
      ...
      {/* MENÚ */}
      <div className="flex flex-wrap justify-center gap-4 md:gap-8 items-center bg-white/70 backdrop-blur-xl px-6 py-3 md:px-8 rounded-full border border-white/50 shadow-lg shadow-slate-200/50 mb-6 md:mb-0 transform hover:scale-[1.02] transition-transform duration-300">
